...
CHART = os.path.join(OUT_DIR, "charts")
MODEL = os.path.join(OUT_DIR, "models")
REPORT = os.path.join(OUT_DIR, "reports")
...
def load_data():
    ...
    files = [f for f in os.listdir(BASE_DIR) if f.lower().endswith(supported)]
    if not files:
        raise FileNotFoundError("No CSV/XLSX file found in project directory.")
...
title_style = ParagraphStyle(name='title', fontSize=30, leading=36, alignment=TA_CENTER, textColor=HexColor("#22d3ee"), spaceAfter=20)
subtitle_style = ParagraphStyle(name='subtitle', fontSize=18, leading=24, alignment=TA_CENTER, textColor=HexColor("#a78bfa"), spaceAfter=40)
body_style = ParagraphStyle(name='body', fontSize=14, leading=20, spaceAfter=20)

timestamp = time.strftime("%Y%m%d_%H%M%S")
report_path = os.path.join(REPORT, f"Retail_Sales_Report_{timestamp}.pdf")

doc = SimpleDocTemplate(report_path, pagesize=A4, leftMargin=72, rightMargin=72, topMargin=72, bottomMargin=72)
...
joblib.dump(rf, os.path.join(MODEL, "rf_model.pkl"))
joblib.dump(iso, os.path.join(MODEL, "isolation_forest.pkl"))
